    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if datetime.date.today() <= form.cleaned_data["stay_start_date"] < form.cleaned_data["stay_end_date"]:
                ...
                return render(request, "guest-booking.html", {"form": form})
            for each_booking in PerDayBooking.objects.filter(bookingss__room=room,bookingss__status="Accepted").exclude(bookingss__status="Pending").exclude(bookingss__status="Rejected"):
...
def perdaybooking_list(request,id):
    ...
    perday=PerDayBooking.objects.filter(bookingss=b)
